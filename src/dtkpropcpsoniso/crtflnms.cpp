...
void mkFileNames(char ** (&argv), OptionFlags &opts, string &i_fn, string &l_fn,\
      string &p_fn) {
   ...
   if ( opts.outfname ) {
      basename=string(argv[opts.outfname])+string(".xxx");
   }
   char prope='V';
   char propiso='d';
   ...
   p_fn=l_fn;
   FileUtils::ReplaceExtensionOfFileName(p_fn,"pov");
