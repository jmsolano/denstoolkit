...
void WaveFunctionGrid3D::WriteCubeOneElecDiseq(ofstream &ofil,GaussWaveFunction &wf) {
   double xx,yy,zz;
   xx=xin[0];
   yy=xin[1];
   zz=xin[2];
   for (int i=0; i<npts[0]; i++) {
      ...
      for (int j=0; j<npts[1]; j++) {
         zz=xin[2];
         for (int k=0; k<npts[2]; k++) {
            prop1d[k]=wf.EvalOneElecDisequilibrium(xx,yy,zz);
            zz+=dx[2][2];
         }
         WriteCubeProp(ofil,npts[2],prop1d);
         yy+=dx[1][1];
