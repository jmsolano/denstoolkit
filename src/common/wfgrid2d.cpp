...
void WaveFunctionGrid2D::MakeTsv(string &onam,GaussWaveFunction &wf,ScalarFieldType ft) {
   ...
   switch (ft) {
      ...
      case DORI :
         ...
         break;
      case SPND :
         WritePlaneTsvSpinDensity(ofil,wf);
         break;
      case RHO2 :
         WritePlaneTsvOneElecDiseq(ofil,wf);
         break;
      default:
...
bool WaveFunctionGrid2D::WritePlaneTsvSpinDensity(ofstream &ofil,GaussWaveFunction &wf) {
   ofil << scientific << setprecision (10);
   double e1,e2,xx[3];
   e1=-1.0e0;
   for (int i=0; i<npts[0]; i++) {
      ...
      for (int j=0; j<npts[1]; j++) {
         ...
         prop1d[j]=wf.EvalSpinDensity(xx[0],xx[1],xx[2]);
         e2+=dx[1];
      }
      e2=-1.0e0*maxdim;
      for (int j=0; j<npts[1]; j++) {
...
bool WaveFunctionGrid2D::WritePlaneTsvOneElecDiseq(ofstream &ofil,GaussWaveFunction &wf) {
   ofil << scientific << setprecision (10);
   double e1,e2,xx[3];
   e1=-1.0e0;
   for (int i=0; i<npts[0]; i++) {
      ...
      for (int j=0; j<npts[1]; j++) {
         ...
         prop1d[j]=wf.EvalOneElecDisequilibrium(xx[0],xx[1],xx[2]);
         e2+=dx[1];
      }
      e2=-1.0e0*maxdim;
      for (int j=0; j<npts[1]; j++) {
